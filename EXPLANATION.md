...
**Version:** 1.0 â€” Design phase (no implementation)
**Author:** Ahmed Eldaly
**Date:** September 2026

![CARE logomark](Logo_Maker_com_ist_logomaker_20260913_202541_795.png)
...
### Design notes (scanned)

![Hand-written design notes](CamScanner 09-15-2026 00.09.jpg)

The original hand-sketched notes that seeded the architecture (scanned from the notebook this design grew out of).
...
**CARE (Connected Ambulanceâ€“Recordsâ€“Emergency)** is an Integrated Health System (IHS) that fuses these worlds into one platform built on three pillars:
...
- Full autonomous AI diagnosis (deliberately excluded: see Â§5.2 and Â§13)
...
| G2 | Hospital pre-arrival notice before ambulance arrives | â‰¥ 5 minutes when transport â‰¥ 5 min |
...
- **Own record**: views their full EHR: history, lab results, medications, allergies, imaging, visit notes (subject to clinician-sealed notes, Â§7).
...
- **Care circle**: invites relatives/caregivers and doctors to follow their status. Each member gets a *permission tier* (see Â§7.2): e.g., a daughter sees "status: stable / alert raised," not the full psychiatric history.
...
- **Self-triage**: describes symptoms; the triage engine returns urgency level and recommended action (self-care / book GP / urgent care / call ambulance). This is the *same* engine dispatch uses (Â§5.1).
- **Ambulance coordination**: when an ambulance is en route, the patient (or bystander) sees live ETA, and the smart-city corridor is coordinated (Â§9, UC-08).
...
- **Diagnosis**: records diagnoses (ICD-coded). May consult the **diagnosis-assist model**: it presents a *ranked differential* (list of candidate diagnoses with confidence and supporting evidence). **The doctor always makes the final call** (Design Decision D1, Â§13).
- **e-Prescription with CDS**: on every prescription the Clinical Decision Support layer checks: drugâ€“drug interactions, drugâ€“allergy conflicts, drugâ€“condition contraindications (e.g., NSAID + renal failure), dosage vs. age/weight, and duplicate therapy. Alerts are tiered: *info â†’ warning â†’ hard-stop* (hard-stop requires an override reason, which is logged).
- **Complication watch**: for post-operative or chronic patients, the doctor sets watch rules on IoT streams ("alert me if SpOâ‚‚ < 92% for 5 min") and receives escalations.
...
- **Capacity management**: beds by type (ER, ICU, ward, isolation), operating rooms, key equipment (ventilators), specialist availability. Feeds the **Capacity Registry** (Â§5.4).
...
- **MCI mode**: during a declared mass-casualty incident, the hospital reports *surge capacity* (how many red/yellow/green patients it can absorb) and receives distributed casualties (Â§9, UC-10).
...
- **Triage & dispatch**: the shared triage engine scores the case; the operator confirms/overrides and dispatches the *best* unit: nearest **suitable** ambulance (BLS vs. ALS) â†’ toward the *best* hospital (capacity + specialty + distance), not merely the nearest one.
- **Smart-city corridor**: for critical (red) transports, requests a green-light corridor from the traffic-management integration; falls back to GPS-optimized routing + vehicle alerts when the city API is unavailable (Design Decision D4, Â§13).
...
flowchart TB
    ...
    SEC["Cross-cutting: AuthN/AuthZ Â· consent Â· confidential-diagnosis policy Â· break-glass Â· immutable audit"]
...
- **Streaming path (hot):** Device Gateway â†’ vitals stream â†’ anomaly rules â†’ alert bus. Latency-sensitive; must work even when the rest of the platform is degraded.
...
- **Analytical path (cold):** model retraining data, incident retrospectives, population dashboards. Fully de-identified export (Â§7.5).
...
| 4 | Green | Less urgent | Book GP within 24â€“48 h |
| 5 | Blue | Non-urgent | Self-care guidance + optional booking |

Plus: confidence score, top contributing factors ("SpOâ‚‚ trend â†“ 6% over 20 min", "history of COPD"), and recommended resource type (BLS/ALS, specialty).
...
1. Drugâ€“drug interaction (against the patient's *active* medication list, including what other doctors prescribed: this is a unified-record superpower).
2. Drugâ€“allergy (including cross-reactivity classes).
3. Drugâ€“condition contraindication (renal/hepatic function, pregnancy, G6PD, etc.).
4. Dose range vs. age/weight/renal function.
5. Duplicate therapy (same class from two prescribers).

**Alert tiers:** `info` (shown inline) â†’ `warning` (must acknowledge) â†’ `hard-stop` (must type an override justification; justification goes to the audit log and to the patient's primary doctor).

**Complication watch (async):** rules attached to conditions ("post-op day 1â€“3: fever > 38.5Â° + tachycardia â†’ possible sepsis, alert surgeon") evaluated continuously against IoT + nursing entries.
...
1. **GPS routing (always on):** traffic-aware fastest path for ambulance â†’ scene and scene â†’ hospital; live ETA fed to the hospital pre-arrival packet and to the patient/relatives view.
...
| Security | mTLS deviceâ†”gateway, signed firmware attestation, per-device keys |
...
Cardiac (ECG patch, BP), Respiratory (SpOâ‚‚, respiratory rate, smart inhaler), Metabolic (CGM/glucose), Gastro/enteral (pH, feeding pumps), Auditory/neuro (hearing aids with fall/impact detection), Ambient (bed sensors, fall detection cameras: event-only, no raw video leaves the home).
...
3. **Emergency â‰  blanket access.** Emergencies widen access through a controlled, audited break-glass path: never by turning permissions off.
...
sequenceDiagram
    ...
    Note over SEC: Case closes â†’ token revoked automatically
...
- `AI_SUGGESTION` is stored *separately* from `DIAGNOSIS` and linked: preserving the model-suggested vs. human-decided pair for audit and model evaluation (Â§5.2).
- `BED_HOLD` has a TTL and a single-holder constraint (Â§5.4).
...
#### UC-01 Â· Patient onboarding & device pairing
...
3. Patient buys any CARE-Compatible device â†’ scans pairing QR in the app â†’ device certificate validated â†’ paired.
4. Gateway starts ingesting; first 48 h establish the patient's **personal baseline** (resting HR, SpOâ‚‚ range, sleep pattern) used by anomaly rules instead of population-only thresholds.

**Edge:** device fails certification handshake â†’ pairing refused with vendor-support link; never "pair anyway."

#### UC-02 Â· Continuous IoT monitoring (steady state)
...
flowchart LR
    ...
    R -->|all normal| T
    R -->|anomaly| A["Alert bus â†’ UC-03"]
    T --> C["Charts: patient app,<br/>doctor watch-lists"]
```

- Rules operate on **trends**, not single readings ("SpOâ‚‚ declining 6% over 20 min" beats "SpOâ‚‚ = 91 once").
...
#### UC-03 Â· IoT anomaly â†’ graduated escalation

The most important everyday flow. Escalation is a ladder, not a jump to 911:

1. **Level A (info):** minor deviation â†’ notify patient in-app ("your HR has been elevated for an hour: feeling ok?"). Patient response ("exercising") closes it and *teaches the baseline*.
2. **Level B (concern):** sustained deviation or moderate pattern â†’ notify patient + T1 care circle + flag on the treating doctor's watch-list.
3. **Level C (urgent):** dangerous pattern (arrhythmia signature, hypoglycemia trend) â†’ triage engine scores it; if level â‰¤ 2 â†’ **auto-open emergency case** in dispatch queue with a *challenge window*: the app rings the patient for 30 s ("Are you OK? Tap to cancel"). No response, or fall + no response â†’ case proceeds as UC-07 with `origin=iot_auto`.
4. Every escalation carries the vitals window that caused it: dispatch sees the *why*, not just "alarm."

**Edge: false alarm handling:** patient cancels within the window â†’ case closes as `false_alarm`, rule sensitivity for that pattern is reviewed; repeated false alarms auto-schedule a device-fit check rather than silently desensitizing.
...
3. Actions wired end-to-end: level 4 â†’ booking screen pre-filtered to suitable doctors; level 2 â†’ "Call ambulance now" button that opens UC-07 with the triage context attached (dispatch does not start from zero).
4. Every self-triage is stored; if the patient books, the doctor sees what the engine said and why.

**Safety rule:** self-triage output never *discourages* seeking care ("you're probably fine") for red-flag symptom combinations: those short-circuit to level â‰¤ 2 phrasing regardless of model confidence.

#### UC-05 Â· Appointment booking & follow-ups
...
3. Follow-up plans (post-op, chronic) generate recurring bookings + the IoT watch rules of Â§5.3.
4. No-show and overbooking policies are facility-configurable.

#### UC-06 Â· Doctor consultation, diagnosis & e-prescription (with CDS)
...
sequenceDiagram
    ...
    alt hard-stop
        ...
        CDS->>EHR: Log override â†’ audit + primary doctor notified
    end
...
#### UC-07 Â· Individual emergency: SOS / IoT-auto / bystander (single flow, three origins)
...
flowchart TB
    ...
    TR --> SEL["Unit + destination selection:<br/>nearest SUITABLE ambulance times<br/>best hospital (capacity registry + specialty + ETA)"]
    ...
    DSP --> ENR["En route: UC-08 corridor if level â‰¤ 2"]
    ...
    HO --> CL["Case closed â†’ holds released,<br/>break-glass tokens revoked, care circle notified"]
```
...
**Detail: care circle during the case:** T1 members get "Emergency in progress: ambulance ETA 6 min â†’ City Hospital"; location sharing to T0/T1 activates *only* for the duration of the case (Â§7.2).

#### UC-08 Â· Ambulance movement coordination (smart-city corridor)
...
sequenceDiagram
    ...
    alt Triage level 1â€“2 AND city integration available
...
#### UC-09 Â· Hospital pre-arrival & handover
...
2. Charge nurse acknowledges â†’ assigns team; team members get pinged with the packet.
3. Diversion path: if the hospital's state collapses meanwhile (e.g., resus bed lost), it can request diversion â†’ dispatch re-runs selection â†’ new hold placed â†’ old hold released â†’ paramedic rerouted with reason. Diversions are counted per facility (registry accuracy accountability).
...
#### UC-10 Â· MCI declaration â†’ field triage â†’ multi-hospital distribution
...
flowchart TB
    ...
    YEL --> DIST
    GRN --> DIST
    DIST["Distribution engine:<br/>match tags â†’ surge commitments<br/>(no hospital overloaded, specialty-aware,<br/>red spread across trauma-capable sites)"]
    DIST --> TRANS["Transports assigned; corridors requested for red"]
    TRANS --> BOARD["Live incident board:<br/>every tag: location â†’ transport â†’ hospital â†’ status"]
    BLK --> BOARD
    BOARD --> IDENT["Identification loop: ID scan / relative inquiry portal<br/>tag â†” patient record linked retroactively"]
    IDENT --> CLOSE["Incident closed â†’ retrospective report auto-compiled"]
```
...
#### UC-11 Â· MCI live command board

The incident commander's single screen: map of scene sectors; per-tag pipeline status (tagged â†’ awaiting transport â†’ en route â†’ arrived â†’ treated); per-hospital load vs. commitment gauges; ambulance positions; unassigned-red alarm (any red without a transport assignment for > X min flashes). All updates are event-driven from paramedic apps, ambulance GPS, and hospital consoles: the board is a *view*, not a data-entry surface, so it stays truthful under chaos.

### Group D : Cross-cutting flows

#### UC-12 Â· Break-glass access : see Â§7.4 (sequence diagram there).

#### UC-13 Â· Care-circle management

Invite by phone/QR â†’ invitee accepts â†’ patient assigns tier (T0/T1) â†’ revocable anytime, one tap. Minors: guardians hold T1â€“T2 by default until majority, then control flips to the patient. An adult patient's tiers are never silently upgraded: every change notifies the patient.

#### UC-14 Â· Capacity update (adapter + manual) : see Â§5.4.

Adapter path: HIS event (admit/discharge/transfer) â†’ Capacity Adapter â†’ registry (seconds). Manual path: tablet console â†’ registry with freshness stamp â†’ staleness decay â†’ dispatch sees the timestamp on every number it trusts.

#### UC-15 Â· Sealed-section grant

Patient opens Privacy Center â†’ sees sealed domains â†’ grants a named clinician T3 on one domain â†’ clinician notified â†’ grant listed and revocable. Every T3 read is logged and visible to the patient ("Dr. X viewed your psychiatric history on ...").

#### UC-16 Â· Post-incident / post-case review

Case closes â†’ auto-compiled timeline (all events, timestamps, decisions, overrides, break-glass uses) â†’ available to: the patient (their own cases), facility QA, dispatch QA, and: de-identified: to the model-evaluation pipeline (triage accuracy vs. outcome; diagnosis-assist agree/disagree analysis).
...
stateDiagram-v2
    ...
    Dispatched --> Diverted : destination collapse â†’ re-select + new hold
    Diverted --> Dispatched
...
`Declared â†’ Surge-collection (parallel with field response) â†’ Active (tagging/distribution/transport loops) â†’ Winding-down (no untransported red/yellow) â†’ Closed (all tags at terminal status) â†’ Retrospective published.` Re-escalation allowed: a Winding-down incident returns to Active if a secondary event occurs (e.g., structure collapse aftershock).
...
| E3 | **False IoT alarm** | Challenge window (UC-03); post-cancel rule review; repeated pattern â†’ device-fit check appointment, never silent desensitization. |
...
| E5 | **Unidentified casualty** | Case runs on the tag/temporary ID (nullable patient link, Â§8); Emergency Summary unavailable â†’ CDS runs in "unknown-history" mode (conservative dosing, allergy-unknown flags); identification loop links retroactively. |
...
| E7 | **Network partition: hospital adapter down** | Registry marks facility stale; dispatch sees timestamps; facility falls back to manual console (Â§5.4). |
...
| E11 | **Conflicting triage scores** (self vs. dispatch vs. ER) | Never silently overwritten: each stored with its input snapshot; deltas displayed with contributing-factor diff (Â§5.1). |
| E12 | **Two dispatchers, one bed** | Impossible by design: bed holds are strongly consistent single-holder (Â§5.4). |
...
| E14 | **Sealed diagnosis vs. drug safety** | Safety override of Â§7.3: interaction fires without revealing the diagnosis. |
...
| E17 | **Ambulance breakdown mid-transport** | Unit flags itself out-of-service â†’ case auto-returns to selection with `in_progress` priority â†’ nearest unit re-tasked â†’ hospital ETA updated. |
...
|---|---|
| **Availability** | Emergency path (intake â†’ triage â†’ dispatch â†’ routing) engineered for 99.99%; degraded modes defined for every dependency (Â§11). Clinical path 99.9%. |
| **Latency** | SOS â†’ case visible in dispatch queue: < 3 s. Vitals device â†’ alert evaluation: < 5 s. Capacity query at dispatch: < 500 ms. |
...
- **Cross-region federation**: protocol for two CARE deployments (or CARE â†” foreign system) to exchange a traveling patient's record with consent.
